...
Preregistration link: https://osf.io/bt2rq
...
Because I was relatively new to R and Github at the beginning of this project, one of the primary challenges involved my gaining the required knowledge and skills necessary for handling data and carrying out data analyses. However, I was confident that, with the skills we were learning in class as well as my having a strong background in STATA (and perhaps more importantly, experience with using Google to troubleshoot issues in STATA), that this would be an appropriately difficult challenge. From a data collection standpoint, I thought it would be challenging to ensure that the sample size is not affected substantially by exclusions due to failed attention checks; however, by being thoughtful about survey design and crafting a clear MTurk HIT description, I thought this issue could be minimized. Finally, I thought it might be a slight challenge to stay within budget, given the relatively high compensation rate used in the original study. However, I tried my best to use the same payment ($0.50 per participant) in order to maintain as much consistency with the original study as possible. 
...
Using an effect size of f = 0.2526, the sample size needed to achieve 80% power was calculated via G*Power. The calculated sample size was 156 participants. This is remarkably close to the sample size used in the original study (where N = 160). Given that running this study with 156 participants already exceeded the funds allotted to this project, the goal for the present replication was to to recruit 156 participants. (While additional participants could have been recruited in order to achieve greater power, this was not feasible within the budget constraints of the present project.)

### Planned Sample

The original study was carried out with 160 participants (who were paid $0.50 each) on Amazon Mechanical Turk. Participants (N = 156, as determined by a power analysis) in the present replication were recruited in the same manner. Only adults (ages 18 and older) living in the United States were eligible to participate. 

In the original study, participants who did not pass two initial attention check items (which comprised the first part of the survey and occurred prior to condition assignment) were directed to the end of the survey (it is unclear whether they received payment). Because class policy specifies that payment of participants is not contingent on their performance or completion of all survey questions, I planned to pay participants who began the survey but failed the initial attention check items; however, consistent with the original study, I planned to redirect these participants to the end of the survey immediately after attention check failure. Therefore, they would not be assigned to condition and would not be included in the final sample. (In the original study, while 160 were recruited, 5 failed the attention check, leaving a total of 155 participants.)

I expected that the demographics of the sample recruited for replication would be extremely similar to that of the sample recruited for the original study (with perhaps some differences due to chance). The original sample was 39% female and had a mean age of 30 years (SD = 8.96 years).
...
Per the original study, participants were excluded if they failed either (or both) of the initial two attention check items. There were no other exclusion criteria.
...
Simple standardized linear regressions were also used in the original study, with contrasts between conditions (represented by standardized beta coefficients and their corresponding p-values) used to indicate differences (or a lack thereof). By employing a condition contrast within a simple standardized linear regression model as my key analysis of interest, I was able to directly compare my findings with those of the original authors.

As an additional note, while an ordinal regression might be considered more appropriate for data comprised of Likert scale ratings, it is common practice to treat such ratings as continuous (rather than ordinal) variables and enter them into a regression model accordingly. A simple linear regression, conducted for each ethicality index, the trust measure, as well as the likelihood of future negotiation measure, allowed me to determine the extent to which participants' ratings for each of these measures differed depending on whether the Buyer was honest, paltered, or lied by commission.

#### Other Planned Analyses

In addition to the simple standardized linear regression model that contains the key analysis of interest, I planned to run three simple linear regressions to assess the effect of condition on participants' ratings of the unethicality of the Buyer's answer (which, like the Buyer unethicality rating, is also in the form of an "ethicality index" representing an average across three items assessing the extent to which they believe the Buyer is "unethical," "dishonest," and "immoral"), their trust in the Buyer, and their likelihood of engaging in a future negotiation with the Buyer. 

### Differences from Original Study

There were no forseeable differences in the sample, with the exception of those due to chance, as I used the same recruitment pool utilized by the original authors (i.e., MTurk workers). The survey was conducted via the same survey platform used in the original study (Qualtrics). 

In addition, I did not collect demographic data on ethnicity, student status, and whether English is their first language. While these questions were asked of participants in the original study, the authors reported in their paper that these were not analyzed. Therefore, in the interest of keeping the survey as brief as possible, I chose to exclude these measures from the replication. I did not anticipate that this will affect the results of the replication, as these measures came at the end of the survey and would therefore have not had an effect on participants' responses to the key DVs of interest.

In addition, while participants were  asked to rate the extent to which they found the Buyer's answer "creative," "innovative," or "original," the effect of condition on these ratings were not analyzed. The original paper did not report analyses on these ratings; however, I chose to include them in the procedure because they preceded key measures assessing trust and likelihood of negotiating with the same Buyer again. It is possible that having to consider aspects of the Buyer's answer, such as its "creativity," may have influenced responses on these other measures in the original study. Therefore, I decided to retain the measures in the survey (even though the results will not be analyzed) in order to maintain a faithful replication.
...
As planned, 156 participants were recruited and run via MTurk. 4 participants were excluded due to attention checks (which occurred prior to condition assignment), leaving 152 participants who were included in analyses. The demographics for this sample were very similar to those in the original study. Participants were 28% female with a mean age of 34 years (SD = 9.48 years). Participants were each paid 60 cents for their time; this payment was based on California minimum wage ($7.25/hour) and an estimated study duration of 5 minutes.
...
Data were prepared following the analysis plan.
...
Below are the sample demographics. As shown in the tables below (and mentioned in the post data collection addendum above), the sample was 28% female, with a mean age of 24 years (SD = 9.48 years).

```{r}
# assessing demographics

gender_summary <- recoded_data %>% 
  group_by(Gender) %>% 
  summarise(n = length(Sub_ID)) %>% 
              mutate(percentage = (n / sum(n))*100) # creates dataframe with percentage breakdown for gender of participants

gender_summary %>% 
  kable() # generates table
              
age_summary <- recoded_data %>% 
  mutate(Age = as.numeric(as.character(Age))) %>% 
  summarise(mean_age = mean(Age),
            SD_age = sd(Age)) # creates dataframe with summary stats (mean and SD) for age of participants
...
The analyses were run as specified in the analysis plan. A simple linear regression was conducted in order to determine the effect of condition (if any) on participants' ratings of the Buyer's ethicality. Additional (non-key) linear regressions were also run to assess the effect of condition on participants' ratings of the ethicality of the Buyer's answer, their trust in the Buyer, and their likelihood of engaging in a future negotiation with the Buyer.

Below are simple linear regression models, which examine the relationship between condition and each of the primary DVs. I used the 'lm.beta' package to obtain standardized regression coefficients, which were reported in the original paper. I have included three regression models per DV, with each of the three models having a different condition as the reference (or comparison) group. This was necessary in order to examine all possible contrasts between conditions. Finally, prior to each model, I have provided a table with relevant descriptive statistics.

#### Buyer Ethicality Ratings

Below is a table containing descriptive statistics for Buyer unethicality.
...
Below is a table containing descriptive statistics for unethicality of the Buyer's answer.
...
answer_unethicality_means <- data_with_ans_unethicality_index %>%
 ...
 mutate(answer_unethicality_ns = answer_unethicality_condition_ns$n,
        answer_unethicality_SE = answer_unethicality_SD/sqrt(answer_unethicality_ns))  # creates data frame with condition means, standard devations, standard errors, and Ns for the answer unethicality index

answer_unethicality_means %>% 
  kable() # generates table
```
...
Below is a table containing descriptive statistics for trust in the Buyer.
...
buyer_trust_means <- buyer_trust_data %>%
 ...
 mutate(BuyerTrust_ns = buyer_trust_condition_ns$n,
        BuyerTrust_SE = BuyerTrust_SD/sqrt(BuyerTrust_ns)) # creates data frame with condition means, standard devations, standard errors, and Ns for Buyer trust
...
Below is a table containing descriptive statistics for likelihood of engaging in a future negotiation with the Buyer.
...
future_negotiation_means <- future_negotiation_data %>%
 ...
 mutate(FutureNegotiation_ns = future_negotiation_condition_ns$n,
        FutureNegotiation_SE = FutureNegotiation_SD/sqrt(FutureNegotiation_ns)) # creates data frame with condition means, standard devations, standard errors, and Ns for future negotiation

future_negotiation_means %>% 
  kable() # generates table
```
...
To compare the findings from the present replication to the findings from the original study, side-by-side plots were generated.
...
Follow-up exploratory analyses were conducted to explore whether participants' ratings of Buyer unethicality mediated the effect of condition on their 1) trust in the Buyer and 2) their likelihood of engaging in a future negotiation with the Buyer. While the original authors did not conduct these mediation analyses for Study 3 (the study targeted for replication in the present project), they did conduct a similar analysis in Study 1. In that study, the authors found that participants' opted to mislead a negotiation partner more often when they had to choose between paltering or honesty (vs. when they had to choose between lying or honesty); a mediation analysis carried out using the Sobel-Goodman bootstrapping method revealed that this effect was mediated by participants' perceived ethicality of the misleading option (either paltering or lying). (It is worth noting that, while the study I replicated did not find differences in participants' ratings of the ethicality between the paltering and lying conditions, this is because participants in the present study were the *targets* of deception rather than the ones engaging in it themselves.)

For the present study, I decided to examine whether ratings of the unethicality of the Buyer mediated trust in the Buyer, as well as likelihood of future negotiation with the Buyer. The authors posit that those who palter or lie, compared to those who are honest, incur reputational costs (i.e., are perceived as more unethical). A negative reputation for being deceptive, in turn, has consequences for how much others trust these individuals as well as how much others are willing to negotiate with them in the future. This suggests that Buyer unethicality should, at least in part, mediate the effect of condition (whether the Buyer is honest, lies, or palters) on ratings of trust in the Buyer as well as likelihood of engaging in a future negotiation with them.

To conduct these mediation analyses, I first created a dataframe containing the measures of interest.

```{r}
# creating a dataframe for the purposes of mediation analysis, which includes the following measures: Buyer ethicality, trust in Buyer, and likelihood of future negotiation 

mediation_data <- recoded_data %>% 
    select(-contains("AnswerUnethical")) %>% # removes answer unethicality items, which will not be used in the exploratory mediation analyses
    pivot_longer(cols = starts_with("BuyerUnethical"),
    names_to = "Buyer_Unethical_Question",
    names_prefix = "BuyerUnethical_",
    values_to = "BuyerUnethicalityRating",
    ...
    group_by(Sub_ID) %>% 
    mutate(BuyerUnethicalityIndex = mean(BuyerUnethicalityRating, na.rm = TRUE)) %>% # creates column with average across the three items re: the unethicality of the buyer's answer
    filter(Buyer_Unethical_Question == 1) %>% # removes repeated data
    select(-Buyer_Unethical_Question, -BuyerUnethicalityRating) %>%  # removes columns referencing individual items and ratings for Buyer unethicality, which are now unnecessary since mediation analyses will be carried out with the average across these items (the unethicality "index") for each participant
    mutate(Condition = ifelse(Condition == "Honest", 0, 
                              ifelse(Condition == "Lie", 1,
                                     ifelse(Condition == "Palter", 2)))) # recodes condition using numeric values, as these are required by the mediate function that will be used for mediation analysis (see below)
```

I then used a "set seed" function, as all mediation analyses involve simulations (and would therefore vary slightly with each run unless "set seed" had been used).

```{r}
set.seed(123) # using set seed for reproducibility (since mediation analysis involves simulations)
```

Using the "mediate" function from the "mediation" package (Tingley et al., 2019), I first created a model assessing the extent to which ratings of Buyer unethicality mediate the effect of condition on ratings of trust in the Buyer. Please note that, for all models I have generated using the "mediate" function, I have used the default number of simulations (1000) and the nonparametric bootstrap type. 

```{r, message = FALSE}
# using mediation package to conduct mediation analysis for trust in Buyer

model_m <- lm(BuyerUnethicalityIndex ~ Condition, data = mediation_data) # regresses the mediator (Buyer unethicality) on the independent variable (Condition)

model_y_trust <- lm(BuyerTrust ~ BuyerUnethicalityIndex + Condition, data = mediation_data) # regresses the outcome variable (BuyerTrust) on both the mediator (Buyer unethicality) and the independent variable (Condition)

med_model_BuyerTrust <- mediate(model_m, model_y_trust, treat = "Condition", mediator = "BuyerUnethicalityIndex", boot = TRUE, boot.ci.type = "perc", sims = 1000) # generates mediation model 
```

The summary below shows the summary of the mediation model for trust in the Buyer. (Please note that, in all summaries and plots pertaining to mediation analyses, "ACME" refers to the average causal mediation effect and "ADE" refers to the average direct effect.)

Both the indirect and direct effects are significant, suggesting partial mediation of Buyer unethicality on the relationship between condition and trust in the Buyer. This is consistent with the authors' claim that at least part of the reason why participants trust the Buyer less when they palter or lie (vs. when they are honest) is due to their perceiving the Buyer as more unethical.

```{r, message = FALSE}
print(summary(med_model_BuyerTrust)) # prints summary of mediation analysis
```

The plot below depicts the results summarized above. The 95% bias-corrected confidence intervals for the indirect effect (ACME) and the direct-effect (ADE) do not contain zero, providing support for partial mediation.

```{r, message = FALSE, warning = FALSE, results = 'hide'}
print(plot(med_model_BuyerTrust,
           main = "Mediation Model for Trust in Buyer",
           xlim = c(-1.5, .5),
           xlab = "Estimate (95% CIs)")) # generates plot with point estimates, 95% CIs, and p-values for the ACME (average causal mediation effect), ADE (average direct effect), and total effect
```

I then created a model assessing the extent to which ratings of Buyer unethicality mediate the effect of condition on ratings of the likelihood of engaging in a future negotiation with the Buyer.

```{r, message = FALSE}
# using mediation package to conduct mediation analysis for likelihood of future negotiation

model_y_future_negotiation <- lm(FutureNegotiation ~ BuyerUnethicalityIndex + Condition, data = mediation_data) # regresses the outcome variable (FutureNegotiation) on both the mediator (Buyer unethicality) and the independent variable (Condition)

med_model_FutureNegotiation <- mediate(model_m, model_y_future_negotiation, treat = "Condition", mediator = "BuyerUnethicalityIndex", boot = TRUE, boot.ci.type = "perc", sims = 1000) # generates mediation model
```

The summary below shows the summary of the mediation model for likelihood of engaging in a future negotiation with the Buyer.

As with the mediation model for trust in the Buyer, the indirect and direct effects are significant, suggesting partial mediation of Buyer unethicality on the relationship between condition and likelihood of engaging in a future negotiation with the Buyer. This is again consistent with the authors' claim; that is, at least part of the reason why participants report being less likely to engage in a future negotiation with the Buyer when they palter or lie (vs. when they are honest) is due to their perceiving the Buyer as more unethical.

```{r, message = FALSE}
print(summary(med_model_FutureNegotiation)) # prints summary of mediation analysis
```

The plot below depicts the results summarized above. Again, the 95% bias-corrected confidence intervals for the indirect effect (ACME) and the direct-effect (ADE) do not contain zero, providing support for partial mediation.

```{r, message = FALSE, warning = FALSE, results = 'hide'}
print(plot(med_model_FutureNegotiation,
           main = "Mediation Model for Future Negotiation",
           xlim = c(-1, 0.5),
           xlab = "Estimate (95% CIs)")) # generates plot with point estimates, 95% CIs, and p-values for the ACME (average causal mediation effect), ADE (average direct effect), and total effect
```

Taken together, the mediation analyses reported above support the authors' claim that the influence of whether the Buyer was honest, lied, or paltered on participants' ratings of trust and likelihood of engaging in a future negotiation with the Buyer was due, in part, to perceptions of Buyer unethicality (which we know, from the confirmatory analyses, were higher in the deception conditions and lower in the honesty condition). This suggests that, when deception is discovered in a negotiation context, associated negative outcomes (i.e., lower trust and likelihood of future negotiation) are at least partially driven by negative reputational consequences (i.e., higher perceptions of unethicality).

#### Supplementary Exploratory Analyses

In conducting the mediation analyses reported above, I read the documentation for the "mediation" R package (Tingley et al., 2019), which stated that the "mediate" function is designed primarily to compare two conditions at a time (typically a "control" condition and a "treatment" condition). Since the present replication dealt with three conditions (honesty, paltering, and lying), I decided to run additional mediation models per the instructions in Section 3.3, entitled "Non-binary treatment variables". This section describes how to run a mediation model for a specific contrast between two levels of a condition variable; by running a model for each possible contrast (or certain contrasts of interest), one can examine the mediation effect (if any) and compare this across different contrasts.

It is important to note that the mediation models below, like the ones described above, examine whether the effect of condition on trust in the Buyer and likelihood of engaging in a future negotiation with the Buyer is mediated by ratings of Buyer unethicality. However, these models take specific levels within the condition variable and examine each possible combination of them (honesty/lying, lying/paltering, and honesty/paltering).

Beginning with the effect of condition on trust in the Buyer, here is a summary of the mediation model containing the honesty/lying contrast:

```{r, message = FALSE}
# using mediation package again, but this time including specific condition contrasts

med_model_BuyerTrust_honest_lie <- mediate(model_m, model_y_trust, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 0, treat.value = 1, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_BuyerTrust_honest_lie)) # prints summary of mediation analysis
```

And here is a summary of the model containing the lying/paltering contrast:

```{r, message = FALSE}
med_model_BuyerTrust_lie_palter <- mediate(model_m, model_y_trust, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 1, treat.value = 2, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_BuyerTrust_lie_palter)) # prints summary of mediation analysis
```

And finally, here is a summary of the model containing the honesty/paltering contrast:

```{r, message = FALSE}
med_model_BuyerTrust_honest_palter <- mediate(model_m, model_y_trust, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 0, treat.value = 2, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_BuyerTrust_honest_palter)) # prints summary of mediation analysis
```

As you can see from the models above, they all show significant indirect and direct effects, confirming the findings from the original mediation model showing that Buyer unethicality partially mediates the effect of condition on trust in the Buyer.

The models below examine the same condition contrasts, but instead examine the effect of condition on participants' reported likelihood of engaging in a future negotiation with the Buyer (and whether this is mediated by Buyer unethicality).

First, here is a summary of the mediation model containing the honesty/lying contrast:

```{r, message = FALSE}
# using mediation package again, but this time including specific condition contrasts

med_model_FutureNegotiation_honest_lie <- mediate(model_m, model_y_future_negotiation, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 0, treat.value = 1, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_FutureNegotiation_honest_lie)) # prints summary of mediation analysis
```

And here is a summary of the model containing the lying/paltering contrast:

```{r, message = FALSE}
med_model_FutureNegotiation_lie_palter <- mediate(model_m, model_y_future_negotiation, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 1, treat.value = 2, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_FutureNegotiation_lie_palter)) # prints summary of mediation analysis
```

And finally, here is a summary of the model containing the honesty/paltering contrast:

```{r, message = FALSE}
med_model_FutureNegotiation_honest_palter <- mediate(model_m, model_y_future_negotiation, treat = "Condition", mediator = "BuyerUnethicalityIndex", control.value = 0, treat.value = 2, boot = TRUE, sims = 1000) # gives contrast between 'honest' and 'lie' conditions

print(summary(med_model_FutureNegotiation_honest_palter)) # prints summary of mediation analysis
```

Again, as you can see from these models, they all show significant indirect and direct effects, confirming the findings from the original mediation model showing that Buyer unethicality partially mediates the effect of condition on reported likelihood of engaging in a future negotiation with the Buyer.

Finally, as another supplementary exploratory analysis, I conducted the same mediation analyses using the "lavaan" package, which allowed me to examine the mediation effect without having to look at the separate condition contrasts. 

Below is the mediation model for trust in the Buyer:

```{r, message = FALSE, warning = FALSE}
# using lavaan package to conduct mediation analysis

buyer_trust_model <- '
              BuyerUnethicalityIndex ~ a*Condition
              BuyerTrust ~ b*BuyerUnethicalityIndex + c*Condition
 
             indirect := a*b
             direct   := c
             total    := c + (a*b)
' # generates regression models for mediation analysis

med_fit_buyer_trust <- sem(buyer_trust_model, data = mediation_data, se = "bootstrap") # generates mediation model

parameterEstimates(med_fit_buyer_trust, boot.ci.type = "bca.simple", level = 0.95, ci = TRUE, standardized = FALSE) %>% 
  kable() # creates table summary of mediation analysis
```

And here is the model for likelihood of future negotiation:

```{r, message = FALSE, warning = FALSE}
# using lavaan package to conduct mediation analysis

future_negotiation_model <- '
              BuyerUnethicalityIndex ~ a*Condition
              FutureNegotiation ~ b*BuyerUnethicalityIndex + c*Condition
...
med_fit_future_negotiation <- sem(future_negotiation_model, data = mediation_data, se = "bootstrap") # generates mediation model

parameterEstimates(med_fit_future_negotiation, boot.ci.type = "bca.simple", level = 0.95, ci = TRUE, standardized = FALSE) %>% 
  kable() # creates table summary of mediation analysis
```

Again, both of these models show significant direct and indirect effects, suggesting that Buyer unethicality partially mediates the effect of condition on 1) trust in the Buyer and 2) reported likelihood of engaging in a future negotiation with the Buyer.

## Discussion

### Summary of Replication Attempt

To conclude, the key finding that individuals evaluate those who palter and those who lie by commission as being equally unethical was successfully replicated. In the original study, the contrast between the paltering and lying conditions in simple linear regression assessing the effect of condition on ratings of the unethicality of the Buyer indicated a non-significant p-value (p = .91); this p-value was also non-significant in the present replication (p = .90). This finding not only demontrates that participants view those who palter and lie as being equally unethical, but it also provides indirect support for the original authors' broader claim that judgments of those who palter give more weight to the intent to mislead rather than the truth value of the statement (which is reflected in palterers being judged as severely as those who outright lie). 

### Commentary

Follow-up exploratory analysis  provide support the original authors' claim that, in a negotiation context, the negative reputational consequences of lying and paltering (i.e., being perceived as more unethical relative to those who are honest) are one reason why individuals trust liars and palterers less and report being less likely to engage in a future negotiation with them. Specifically, mediation analyses revealed that ratings of Buyer unethicality partially mediate the relationship between condition (honesty, lying, or paltering) and trust in the Buyer, as well as the relationship between condition and likelihood of engaging in a future negotiation with the Buyer. This suggests that the effect of whether one's negotiation partner is deceptive or honest on one's trust in that individual is driven, in part, by one's perceptions of how unethical that individual is; the same can be said for one's reported likelihood of engaging in a future negotiation with that individual.

These findings, along with the findings from the confirmatory analysis, highlight the risks of engaging in paltering as a negotiation strategy. While paltering might seem less unethical than lying by commission from a first-person perspective (i.e., when making decisions about how one should proceed in a negotiation; Study 1, Rogers et al., 2017), the key finding replicated in the present report suggests that, if one's intent to mislead is discovered, one could be judged with the same severity as they would have been if they had lied. This has interesting implications for future work, which should examine how paltering is evaluated relative to lying in different contexts (e.g., in the moral domain).
