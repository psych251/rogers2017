...
A more precise planned sample size will be computed via power analysis. The original paper does not report effect sizes, which will be necessary for determining the sample size needed for achieving at least 80% power. Once effect sizes have been computed (utilizing group means reported in the original paper), I will compute the sample sizes needed to achieve 80%, 90%, or 95% power, respectively. 

At a minimum, I will plan to recruit enough participants to achieve 80% power; however, if running a sample size large enough to achieve 90% or 95% power is feasible (given budget constraints), I will consider doing so. Furthermore, my choice of sample size will depend, in part, on the power achieved in the original study (which I will also calculate once I have the computed effect sizes). I will aim to reach the same level of power that was achieved in the original study (unless the study was overpowered, in which case, I will aim for between 80% and 95% power).
...
buyer_unethicality_model_LieRef_standardized
...
buyer_unethicality_model_HonestRef_standardized
```
...
buyer_unethicality_model_PalterRef_standardized
```
...
ans_unethicality_model_LieRef_standardized
```
...
ans_unethicality_model_HonestRef_standardized
```
...
ans_unethicality_model_PalterRef_standardized
```
...
BuyerTrust_model_LieRef_standardized
```
...
BuyerTrust_model_HonestRef_standardized 
```
...
BuyerTrust_model_PalterRef_standardized 
```
...
Future_Negotiation_model_LieRef_standardized 
```
...
Future_Negotiation_model_HonestRef_standardized 
```
...
Future_Negotiation_model_PalterRef_standardized
```
...
# plots based on data from original study (taken from means and SDs reported in Table 3 of Rogers et al., 2017)
...
side_by_side_buyer_unethicality_plots
...
side_by_side_FutureNegotiation_plots
```
