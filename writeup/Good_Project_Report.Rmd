...
Since effect sizes were not reported in the original paper, I calcuated effect sizes using the group means and standard deviations reported in Table 2 (Rogers et al., 2017). Since the standard deviation was not reported for the mean rating of the unethicality of the Buyer's answer in the Honest condition, I calcuated the effect size of each condition contrast (Honest vs. Palter, Honest vs. Lie by Commission, and Palter vs. Lie by Commission) for the other three measures (rating of the unethicality of the Buyer, trust in the Buyer, and likelihood of future negotiation). Most of the calculated effect sizes were in the medium range (Cohen's d = ~0.5 for each).

Using Cohen's d = 0.5, the sample size needed to achieve 80% power was calculated via G*Power. The calculated sample size was 159 participants. Since this is incredibly close to the sample size used in the original study (where N = 160), the goal for the present replication will be to also recruit 160 participants. (While additional participants could be recruited in order to achieve greater power, this is not feasible within the budget constraints of the present project and is not necessary, given that a similar power level was achieved by the original study.)

### Planned Sample

The original study was carried out with 160 participants (who were paid $0.50 each) on Amazon Mechanical Turk. Participants (N = 160, as determined by a power analysis) will be recruited in the same manner. Only adults (ages 18 and older) living in the United States will be eligible to participate. 
...
The key analysis of interest is a simple linear regression, which will be used to assess the effect of condition (paltering, lying by commission, or honesty) on participants' ratings of the ethicality of the Buyer's answer, the ethicality of the Buyer themselves, their trust in the Buyer, and likelihood of engaging in a future negotiation with the Buyer. (Please note that "ethicality" ratings included in these analyses will be an average of participants' ratings of the extent to which they believe the Buyer [or the Buyer's answer] is "unethical," "dishonest," and "immoral". This procedure was used in the original study, which referred to these averages as "ethicality indexes".)

A simple linear regression was chosen due to its being the analysis used in the original study. By using this analysis, I will be able to directly compare my findings with those of the original authors.

While an ordinal regression might be considered more appropriate for data comprised of Likert scale ratings, it is common practice to treat such ratings as continuous (rather than ordinal) variables and enter them into a regression model accordingly. A simple linear regression, conducted for each ethicality index, the trust measure, as well as the likelihood of future negotiation measure, will allow me to determine the extent to which participants' ratings for each of these measures differed depending on whether the Buyer was honest, paltered, or lied by commission.
...
Please note that I have not included the data preparation code in the report. This code can be found in the Project Report R Markdown file in the 'rogers2017' folder on GitHub.
...
replication_data <- read_csv("https://raw.githubusercontent.com/psych251/rogers2017/master/data/PilotA_Replication_Data.csv") # reading in data from github
...
clean_data <- clean_data %>% 
  ...
  mutate(Condition = ifelse(!is.na(`PalterTiming_First Click`), "Palter", 
                            ifelse(!is.na(`LieTiming_First Click`), "Lie",
                                   ifelse(!is.na(`HonestyTiming_First Click`), "Honest", NA)))) # creates column with condition assignment

clean_data <- clean_data %>% 
  select(-c(contains("Timing"))) # this gets rid of the timing information, which we just needed to get condition assignment

clean_data$Sub_ID = factor(clean_data$Sub_ID)
...
long_recoded_data <- recoded_data %>% 
  ...
  pivot_longer(
    cols = starts_with("BuyerUnethical"),
...
long_recoded_data_numeric_ratings <- long_recoded_data %>% 
  mutate_at(vars(FutureNegotiation, BuyerTrust, AnsUnethicalityRating, BuyerUnethicalityRating), as.numeric) # converts data type of Likert ratings from character to numeric 

buyer_mean_data <- long_recoded_data_numeric_ratings %>% 
  filter(Ans_Unethical_Question == 1)

data_with_buyer_unethicality_index <- buyer_mean_data %>% 
  group_by(Sub_ID) %>% 
  mutate(BuyerUnethicalityIndex = mean(BuyerUnethicalityRating, na.rm = TRUE))

data_with_buyer_unethicality_index$Condition <- as.factor(data_with_buyer_unethicality_index$Condition) # converts condition from character to factor

answer_mean_data <- long_recoded_data_numeric_ratings %>% 
  filter(Buyer_Unethical_Question == 1)

data_with_ans_unethicality_index <- answer_mean_data %>% 
  group_by(Sub_ID) %>% 
  mutate(AnsUnethicalityIndex = mean(AnsUnethicalityRating, na.rm = TRUE))

data_with_ans_unethicality_index$Condition <- as.factor(data_with_ans_unethicality_index $Condition) # converts condition from character to factor

trust_mean_data <- long_recoded_data_numeric_ratings %>% 
  filter(Buyer_Unethical_Question == 1 & Ans_Unethical_Question == 1) 

trust_mean_data$Condition <- as.factor(trust_mean_data$Condition) # converts condition from character to factor

future_negotiations_mean_data <- long_recoded_data_numeric_ratings %>% 
  filter(Buyer_Unethical_Question == 1 & Ans_Unethical_Question == 1) 

future_negotiations_mean_data$Condition <- as.factor(future_negotiations_mean_data$Condition) # converts condition from character to factor
...
The analyses as specified in the analysis plan. Simple linear regressions will be conducted in order to determine the effect of condition (if any) on participants' ratings of the Buyer's ethicality, their trust in the Buyer, and their likelihood of engaging in a future negotiation with the Buyer.
...
trust_mean_data$Condition <- relevel(trust_mean_data$Condition, ref = "Lie") # changes reference group to Lie condition

BuyerTrust_model_LieRef <- lm(BuyerTrust ~ Condition, data = trust_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Lie condition as reference group
...
trust_mean_data$Condition <- relevel(trust_mean_data$Condition, ref = "Honest") # changes reference group to Honest condition

BuyerTrust_model_HonestRef <- lm(BuyerTrust ~ Condition, data = trust_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Honest condition as reference group
...
trust_mean_data$Condition <- relevel(trust_mean_data$Condition, ref = "Palter") # changes reference group to Honest condition

BuyerTrust_model_PalterRef <- lm(BuyerTrust ~ Condition, data = trust_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Palter condition as reference group
...
future_negotiations_mean_data$Condition <- relevel(future_negotiations_mean_data$Condition, ref = "Lie") # changes reference group to Lie condition

Future_Negotiation_model_LieRef <- lm(FutureNegotiation ~ Condition, data = future_negotiations_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Lie condition as reference group
...
future_negotiations_mean_data$Condition <- relevel(future_negotiations_mean_data$Condition, ref = "Honest") # changes reference group to Honest condition

Future_Negotiation_model_HonestRef <- lm(FutureNegotiation ~ Condition, data = future_negotiations_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Honest condition as reference group
...
future_negotiations_mean_data$Condition <- relevel(future_negotiations_mean_data$Condition, ref = "Palter") # changes reference group to Palter condition

Future_Negotiation_model_PalterRef <- lm(FutureNegotiation ~ Condition, data = future_negotiations_mean_data) # linear regression examining relationship between condition (explanatory variable) on ratings of trust in the Buyer (dependent variable), with Palter condition as reference group
...
mydata_buyer_unethicality_plot <- ggplot(buyer_unethicality, aes(x=Condition, y=buyer_unethicality_mean, fill=Condition)) +
  geom_bar(position = "dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=buyer_unethicality_mean - buyer_unethicality_SD, ymax=buyer_unethicality_mean + buyer_unethicality_SD), width = .2, position = position_dodge(.9)) # plot of condition means for ratings of Buyer ethicality

mydata_answer_unethicality_plot <- ggplot(answer_unethicality, aes(x=Condition, y=answer_unethicality_mean, fill=Condition)) +
  geom_bar(position = "dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=answer_unethicality_mean - answer_unethicality_SD, ymax=answer_unethicality_mean + answer_unethicality_SD), width = .2, position = position_dodge(.9)) # plot of condition means for ratings of the ethicality of the Buyer's answer

mydata_BuyerTrust_plot <- ggplot(BuyerTrust_means, aes(x=Condition, y=BuyerTrust_mean, fill=Condition)) +
  geom_bar(position = "dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=BuyerTrust_mean - BuyerTrust_SD, ymax=BuyerTrust_mean + BuyerTrust_SD), width = .2, position = position_dodge(.9)) # plot of condition means for ratings of trust in the Buyer

mydata_FutureNegotiation_plot <- ggplot(FutureNegotiation_means, aes(x=Condition, y=FutureNegotiation_mean, fill=Condition)) +
  geom_bar(position = "dodge", stat="identity") +
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=FutureNegotiation_mean - FutureNegotiation_SD, ymax=FutureNegotiation_mean + FutureNegotiation_SD), width = .2, position = position_dodge(.9))  # plot of condition means for ratings of likelihood of engaging in a future negotiation with the Buyer
```
...
Below are side-by-side plots showing the group means and SDs from the replication data (Plot "A") and the group means and SDs from the original data (Plot "B").

##### Ratings of the Unethicality of the Buyer
```{r, echo=FALSE}
# code for side-by-side plots
side_by_side_buyer_unethicality_plots <- plot_grid(mydata_buyer_unethicality_plot, originaldata_buyer_unethicality_plot, labels="AUTO")
...
Please note that Plot "B" below does not have SD bars for the Honest condition because the group SD was not reported for this measure (see Table 2, Rogers et al., 2017).

```{r, echo=FALSE, warning=FALSE}
side_by_side_answer_unethicality_plots <- plot_grid(mydata_answer_unethicality_plot, originaldata_answer_unethicality_plot, labels="AUTO")
...
```{r, echo=FALSE}
side_by_side_BuyerTrust_plots <- plot_grid(mydata_BuyerTrust_plot, originaldata_BuyerTrust_plot, labels="AUTO")
...
```{r, echo=FALSE}
side_by_side_FutureNegotiation_plots <- plot_grid(mydata_FutureNegotiation_plot, originaldata_FutureNegotiation_plot, labels="AUTO")
